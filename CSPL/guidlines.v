...
Module FlashLuminanceThreshold.
  ...
  (** Note: Rle_dec is used to handle cases where the denominator might be zero. *)
  (** If the sum is non-positive, we define contrast as zero. *)
  (** Otherwise, we compute the absolute difference divided by the sum. *)

  (** Michelson contrast between two frames at pixel (x, y). *)
  (* Note: Rle_dec is used to handle cases where the denominator might be zero. *)
  (* If the sum is non-positive, we define contrast as zero. *)
  (* Otherwise, we compute the absolute difference divided by the sum. *)
