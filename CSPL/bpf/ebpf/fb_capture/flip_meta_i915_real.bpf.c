...
// Use the working i915 approach but try to get real framebuffer inodes
...
int kprobe_i915_commit(struct pt_regs *ctx)
{
    ...
    // Try to find a valid DMA-BUF inode by scanning proc filesystem
    // This is a simplified approach - in reality we'd extract from the request
    rec->inode = 123456; // Use a test inode that we can create
