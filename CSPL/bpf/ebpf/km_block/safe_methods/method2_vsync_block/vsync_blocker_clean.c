...
static int max_blocks = 20;  // Allow longer freeze effect but still safe
...
static int handler_drm_mode_page_flip_ioctl(struct kprobe *p, struct pt_regs *regs)
{
    if (freeze_active) {
        ...
        msleep(300); // 300ms delay creates visible freeze effect
...
static int handler_drm_plane_commit(struct kprobe *p, struct pt_regs *regs)
{
    if (freeze_active) {
        ...
        // SAFE: Add delay instead of blocking
        msleep(150); // Shorter delay for plane updates
