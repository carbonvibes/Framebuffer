...
static int handler_drm_framebuffer_init(struct kprobe *p, struct pt_regs *regs)
{
    ...
    // Capture the framebuffer content (only when not blocking)
...
static int drm_fb_proc_show(struct seq_file *m, void *v)
{
    ...
    // Show frame blocking status
    mutex_lock(&block_mutex);
    seq_printf(m, "Frame blocking: %s\n", block_frames ? "ENABLED" : "DISABLED");
    seq_printf(m, "Blocked frame count: %d\n", blocked_frame_count);
    mutex_unlock(&block_mutex);
    seq_printf(m, "\n");
...
static ssize_t drm_fb_block_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    ...
    if (strcmp(input, "1") == 0 || strncmp(input, "enable", 6) == 0) {
        block_frames = true;
        pr_info("Frame blocking ENABLED\n");
    } else if (strcmp(input, "0") == 0 || strncmp(input, "disable", 7) == 0) {
        block_frames = false;
