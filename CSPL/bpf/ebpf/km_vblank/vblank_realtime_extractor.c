...
// VBlank event handler - Hook the core vblank handler
static int handler_drm_handle_vblank(struct kprobe *p, struct pt_regs *regs)
{
    struct drm_device *dev;
    int pipe;
    ...
    if (atomic64_read(&vblank_events) % 60 == 1) {
        ...
        pr_info("VBlank event on device %p pipe %d, attempting capture...\n", dev, pipe);
        
        // Find the CRTC for this pipe/device
        if (dev->mode_config.crtc_list.next != &dev->mode_config.crtc_list) {
            struct drm_crtc *temp_crtc;
            list_for_each_entry(temp_crtc, &dev->mode_config.crtc_list, head) {
                if (crtc_count == pipe) {
                    crtc = temp_crtc;
                    break;
                }
        ...
        if (crtc) {
            uint32_t sequence = atomic64_read(&vblank_events);
            capture_vblank_fb(crtc, sequence);
        } else {
            pr_info("Could not find CRTC for pipe %d\n", pipe);
        }
    }
...
static struct kprobe kp_vblank = {
    .symbol_name = "drm_handle_vblank",
    .pre_handler = handler_drm_handle_vblank,
};
